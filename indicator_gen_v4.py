...
Huber loss for MSE as well - version 4 -not  run yet
"""
...
def construct_samples(df, window_size=20, horizon=10):
    ...
    Define equilibrium as the average log_close over the input window.
    The target is defined as the normalized difference between the future average log_close
    (over the next 'horizon' days) and the equilibrium:

      target = (future_avg - equilibrium) / ATR_252
    """
    X_samples = []
    y_samples = []
    for symbol in df["symbol"].unique():
        ...
        for i in range(start_idx, len(df_symbol) - horizon):
            ...
            equilibrium = window_data["log_close"].mean()  # average over input window
...
def mean_reversion_loss(predictions, targets, delta=1.0):
    """
    Custom Huber loss (SmoothL1) for mean reversion prediction.
    For small errors (|error| < delta), it uses quadratic loss;
    for large errors, it uses linear loss.
    """
...
# -----------------------------
# Training Function with Gradient Clipping, Learning Rate Scheduling, and Early Stopping
# -----------------------------
...
def train_model(
    ...
    criterion = lambda pred, tgt: mean_reversion_loss(pred, tgt, delta=1.0)
...
def main():
    ...
    model.eval()
    # Use the same Huber loss for final evaluation
    final_losses = []
    with torch.no_grad():
        for X_val, y_val in val_loader:
            X_val = X_val.to(device)
            y_val = y_val.to(device)
            outputs = model(X_val)
            loss = mean_reversion_loss(outputs, y_val, delta=1.0)
            final_losses.append(loss.item())
    avg_val_loss = np.mean(final_losses)
    print(f"Final Validation Loss (Huber): {avg_val_loss:.6f}")
