...
def preprocess_data(df):
    ...
    for symbol in df["symbol"].unique():
        ...
        # Compute previous log_close for True Range calculation
        df_symbol["prev_log_close"] = df_symbol["log_close"].shift(1)

        # Compute log True Range for each day using log-prices
        ...
        # Compute ATR over a 252-day rolling window (simple moving average of log_true_range)
...
def construct_samples(df, window_size=11):
    ...
    and the target is the volatility-adjusted deviation of the current day's log_close from the mean
    of the previous 10 days. Ensures numerical consistency (dividing by exactly 10) and skips samples
    with missing/zero ATR.
    """
    X_samples = []
    y_samples = []
